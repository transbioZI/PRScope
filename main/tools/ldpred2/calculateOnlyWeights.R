...
library(data.table)
obj.bigSNP <- fread(paste0(fileGeno,".bim"),header = F, sep = " ") #TODO

# Store some key variables
CHR <- obj.bigSNP$V1    #TODO
POS <- obj.bigSNP$V4  #TODO
NCORES <- parsed$cores
...
obj.bigSNP = obj.bigSNP[,-3]
colnames(obj.bigSNP) = colMap
map <- obj.bigSNP
...
df_beta <- snp_match(sumstats, map, join_by_pos=!mergeByRsid, match.min.prop=0.2)
...
# LDPRED2-Auto
  ...
  keep = which(is.na(keep) == F)
  beta <- rowMeans(as.data.frame(sapply(multi_auto[keep], function (auto) {
    auto$beta_est
  })))

  h2_est_med <- sapply(multi_auto[keep], function(auto) auto$h2_est)
  p_est <-sapply(multi_auto[keep], function(auto) auto$p_est)
...
saveRDS(ll, paste0(fileOutput,".weights"))
}
