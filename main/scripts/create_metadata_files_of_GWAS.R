...
if(length(studies) != 0) {
  ...
  for(f in studies) {
    ai = which(f == studies)
    res = fread(paste0(path_to_sumstats,"/",f,"/qced/",f,".metadata.tsv"))

    colnames(res) = c("N_failed", "P_failed", "BETA_failed","BETA_failed_reason", "MAF_matching_failed", "HM_prefix_readed","SNP_count_after_QC",
                                "BETA_is_Z_SCORE", "SE_failed", "CHR_failed","SNP_count_before_QC","duplicate_columns_dropped","columns_before_QC","columns_after_QC")
  ...
  all_criteria = data.frame(a = a, b = b, c = c, d = d, e = e, f = f, g = g, k = k)
