...
write.table(df[,-1], paste0(prs_path,"/",name,".tsv"), row.names = F, col.names = T, quote = F)

PRS_data = read.table(paste0(prs_path,"/",name,".tsv"), sep=" ", header=TRUE)
print(dim(PRS_data))
#select(PRS_data, starts_with("Pt_0.0001"))

pca = prcomp(PRS_data[,-1])$x
pdf(paste0(prs_path,"/",name,"_PCA_PRS.pdf"))
pairs(pca[,1:5])
dev.off()
