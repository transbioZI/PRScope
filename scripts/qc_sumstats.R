...
#base <- base %>% group_by(SNPID) %>% slice(which.min(p_value))
print(dim(base))
...
print(length(matches_na))
#### Save data ####

print(dim(current_s))
write.table(current_s[,c("VARID","SNPID","CHR","POS","Allele1","Allele2","p_value","BASEMAF","BETA","OR")], output, quote = F, row.names =F, sep = " ")
